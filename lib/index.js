...
MapboxGeocoder.prototype = {
  ...
  _forwardGeocode: function(geocoderService, config) {
    try {
      console.log('_forwardGeocode', config);
      console.log('v', v);
      ...
      var query = helpers.stringifyBoolean(
        extend(
      ...
      console.log('geocoderService.client', geocoderService.client);

      // return geocoderService.client.createRequest({
      //   method: 'GET',
      //   path: '/geocoding/v1/address',
      //   query: {
      //     key: 'dAbVDTqrFKyPY5qFDmoMFEYli6ISnqpC',
      ...
      if(config.origin == 'eu1.locationiq.com') {
        return axios.get("https://eu1.locationiq.com/v1/search.php", {
          params: {
            key: 'f5cebd535f7e34',
            q: config.query,
            format: 'json'
  ...
  _geocode: function(searchInput) {
    ...
    // make the request a reverseGeocode
      ...
      request = this._forwardGeocode(this.geocoderService, config);

      console.log('_forwardGeocode', this._forwardGeocode(this.geocoderService, config));
      ...
      function(response) {
        ...
        if (!response){
          ...
          if (response.data.results) {
            res = {
              features: response.data.results[0].locations
              .map(function (d) {
                return ({
                  properties: {
                    short_code: d.linkId,
                  ...
                  place_name: response.data.results[0].providedLocation.location
                });
              })
            }
          } else {
            res = {
              features: response.data.map(function (d) {
                return ({
                  bbox: d.boundingbox ? [
